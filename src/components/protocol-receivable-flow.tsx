...
import type { ReceivableCreated } from "@protocol/schemas";
...
export function ProtocolReceivableFlow() {
  ...
  return <div className="receivable-demo-form">
    ...
    <p role="status">{status}</p>{published ? <section className="demo-success"><h2>Recebível publicado</h2><p>ACKs: {published.acknowledgedRelays.join(", ")}</p><Link className="button button--primary" href={`/protocolo/originar?receivable=${published.id}`}>Abrir link do cliente originador</Link></section> : null}
  </div>;
}
