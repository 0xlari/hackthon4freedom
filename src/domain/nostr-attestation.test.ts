...
describe("public Nostr attestations", () => {
  it("contains only the approved positive, opaque schema", () => {
    ...
    const template = buildAttestationTemplate({ subjectPubkey: getPublicKey(secret), assertion: "operation_completed", operationRef: "a".repeat(64), evidenceHash: "b".repeat(64), occurredAt: new Date("2026-07-14T12:00:00.000Z") });
  ...
  it("references the prior event when issuing an append-only correction", () => {
    ...
    const template = buildAttestationTemplate({ subjectPubkey: getPublicKey(secret), assertion: "dispute_resolved", operationRef: "d".repeat(64), evidenceHash: "e".repeat(64), occurredAt: new Date("2026-07-14T12:00:00.000Z"), correctionOf: previous });
